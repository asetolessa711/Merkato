// @allow-hardcode - router contains legacy '/shop' redirect path for back-compat
...
import DealBrowsePage from './pages/browse/DealBrowsePage'';
...
import AdminMarketingManager from './pages/AdminMarketingManager'';
import AdminHeroBanners from './pages/AdminHeroBanners'';
import AdminCategories from './pages/AdminCategories'';


// promo routes are redirected to Marketing hub
...
import ROUTES, { LinkBuilder, normalizeCategorySlug, normalizeSubcategorySlug } from './config/routes'';
  ...
  return (
    <BrowserRouter>
      <MessageProvider>
        ...
        <Routes>
          {/* Back-compat + normalization redirects */}
          {/* Legacy /shop redirects (bookmarks only):
              - /shop?category=foo -> /c/foo?sort=best
              - /shop?search=q -> /search?q=q
              - /shop?sort=deals -> /deals/under-25 */}
          <Route
            path="/shop"
            element={
              (() => {
                try {
                  ...
                  if (cat) {
                    ...
                    return <Navigate to={LinkBuilder.toCategory(c)} replace />;
                  }
                  if (search) {
                    return <Navigate to={LinkBuilder.toSearch(search)} replace />;
                  }
                  if (sort && String(sort).toLowerCase() === 'deals') {
                    return <Navigate to={LinkBuilder.toDeal('under-25')} replace />;
                  }
                } catch {}
          ...
          {/* /c/:Category (mixed case or missing sort) -> lowercase + ?sort=best */}
          <Route
            path="/c/:category"
            element={
              (() => {
                try {
                  ...
                  if (m) {
                    ...
                    const c = normalizeCategorySlug(raw);
                    const sp = new URLSearchParams(window.location.search);
                    const sort = sp.get('sort') || 'best';
                    const target = LinkBuilder.toCategory(c, { sort });
          ...
          {/* /c/:category/:subcat normalization to lowercase + ?sort=best */}
          <Route
            path="/c/:category/:subcat"
            element={
              (() => {
                try {
                  const m = window.location.pathname.match(/^\/c\/([^/?#]+)\/([^/?#]+)/i);
                  if (m) {
                    const rawC = decodeURIComponent(m[1] || '');
                    const rawS = decodeURIComponent(m[2] || '');
                    const c = normalizeCategorySlug(rawC);
                    const s = normalizeSubcategorySlug(rawC, rawS);
                    const sp = new URLSearchParams(window.location.search);
                    const sort = sp.get('sort') || 'best';
                    const target = LinkBuilder.toSubcategory(c, s, { sort });
                    if (target !== window.location.pathname + (window.location.search || '')) {
                      return <Navigate to={target} replace />;
                    }
                  }
          ...
          <Route path={ROUTES.home} element={
            ...
            {/* Note: /shop handled by redirect above */}
            <Route path="discover" element={<DiscoverPage />} />
            <Route path="search" element={<SearchPage />} />
            <Route path="c/:category" element={<CategoryBrowsePage />} />
            <Route path="c/:category/:subcat" element={<CategoryBrowsePage />} />
            <Route path="deals/:dealType" element={<DealBrowsePage />} />
          ...
          {/* Short vendor slug alias renders VendorStore by slug */}
          <Route path={ROUTES.vendorStorefront} element={<VendorStore />} />
          ...
          <Route path={ROUTES.admin} element={
            ...
            {/* Legacy promo routes redirect to Marketing hub */}
            <Route path="promo-codes" element={<Navigate to="/admin/marketing" replace />} />
            <Route path="promo-manager" element={<Navigate to="/admin/marketing" replace />} />
            ...
            {/* Legacy mega-menu page redirects to Marketing hub */}
            <Route path="mega-menu" element={<Navigate to="/admin/marketing" replace />} />
            <Route path="marketing" element={<AdminMarketingManager />} />
            {/* Preserve legacy deep link to metrics by redirecting into Marketing with a query */}
            <Route path="marketing/rails" element={<Navigate to="/admin/marketing?metrics=1" replace />} />
            <Route path="hero-banners" element={<AdminHeroBanners />} />
            {/* Legacy microbanner/trust-ticker routes redirect to Marketing hub */}
            <Route path="microbanner" element={<Navigate to="/admin/marketing" replace />} />
            <Route path="trust-ticker" element={<Navigate to="/admin/marketing" replace />} />
