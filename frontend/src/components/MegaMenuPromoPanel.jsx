...
const DEFAULT_PROMOS = [
  // New Arrivals
  {
    ...
    ctaText: 'Shop New In',
  href: LinkBuilder.toDeal('percent-off-10'),
    type: 'text',
    enabled: true,
  },
  {
  ...
  href: LinkBuilder.toDeal('percent-off-15'),
    type: 'text',
    enabled: true,
  },
  // Top Picks
  ...
  href: LinkBuilder.toDeal('percent-off-20'),
    type: 'text',
    enabled: true,
  },
  {
  ...
  href: LinkBuilder.toDeal('percent-off-25'),
    type: 'text',
    enabled: true,
  },
  // Best Sellers
  ...
  href: LinkBuilder.toDeal('percent-off-30'),
    type: 'text',
    enabled: true,
  },
  {
  ...
  href: LinkBuilder.toDeal('percent-off-40'),
    type: 'text',
    enabled: true,
  },
  // Utility/seasonal
  ...
  href: LinkBuilder.toDeal('percent-off-35'),
    type: 'cta',
    enabled: true,
  },
  {
...
export default function MegaMenuPromoPanel({ activeCategory, activeSubcategory }) {
  ...
  // Prefer explicit promo href; else link to hovered main category; else Discover
  const computedHref = useMemo(() => {
    if (selected?.href) return selected.href;
    const cat = (activeCategory || '').trim();
    const sub = (activeSubcategory || '').trim();
    if (cat && sub) return LinkBuilder.toSubcategory(cat, sub, { sort: 'best' });
    if (cat) return LinkBuilder.toCategory(cat, { sort: 'best' });
    return '/discover';
  }, [selected, activeCategory, activeSubcategory]);
  ...
  onClick={(e) => { if (!computedHref) return; e.preventDefault(); handleClick(); }}
        style={{
