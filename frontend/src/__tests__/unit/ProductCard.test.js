...
// Helper function to render ProductCard with required providers
...
describe('\ud83d\udecd\ufe0f ProductCard Component', () => {
  ...
  test('calls add-to-cart when button clicked', () => {
    ...
    renderProductCard({ product, onAddToCart });
  ...
  test('disables add-to-cart button if out of stock', () => {
    renderProductCard({ product: { ...product, stock: 0 } });
    const btn = screen.getByRole('button', { name: /add to cart/i });
  ...
  test('matches snapshot', () => {
    const { asFragment } = renderProductCard({ product });
