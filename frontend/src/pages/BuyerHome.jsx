    ...
    <div className="p-4">
      ...
      <section className="mb-8">
        ...
        <div className="flex overflow-x-scroll space-x-4">
          {featuredDeals.map(product => (
            <Link key={product._id} to={LinkBuilder.toPdp(product._id)} className="min-w-[180px] bg-white p-2 rounded shadow-md">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link to={LinkBuilder.toCategory('electronics', { sort: 'best' })} className="bg-blue-100 p-4 rounded text-center font-bold">Electronics</Link>
          <Link to={LinkBuilder.toCategory('fashion', { sort: 'best' })} className="bg-pink-100 p-4 rounded text-center font-bold">Fashion</Link>
          <Link to={LinkBuilder.toCategory('home', { sort: 'best' })} className="bg-yellow-100 p-4 rounded text-center font-bold">Home</Link>
          <Link to={LinkBuilder.toCategory('beauty', { sort: 'best' })} className="bg-purple-100 p-4 rounded text-center font-bold">Beauty</Link>
        </div>
      </section>
        ...
        <section className="mb-8">
          ...
          <div className="flex overflow-x-scroll space-x-4">
            {wishlist.slice(0, 3).map(item => (
              <Link key={item._id} to={LinkBuilder.toPdp(item._id)} className="min-w-[180px] bg-white p-2 rounded shadow-md">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {hotDeals.map(product => (
            <Link key={product._id} to={LinkBuilder.toPdp(product._id)} className="bg-white p-4 rounded shadow-md">
              <img src={product.image} alt={product.name} className="h-32 object-cover w-full rounded" />
              <h4 className="font-semibold mt-2 text-center">{product.name}</h4>
