...
export default function CartPage() {
  ...
  <Link to="/discover" className="btn btn-primary" style={{ marginTop: 8, background: '#1B63E0', color: '#fff' }}>Start shopping</Link>
      </div>
    </main>
  );
  ...
  <div className="cart-grid">
        ...
        <section className="cart-items card card--p">
                ...
                <div className="line__main">
                  {it.image && (
                    <img
                      className="line__img"
                      src={it.image}
                      alt={(it.title ? `${it.title}` : `Cart item ${it.sku}`)}
                    />
                  )}
          ...
          <div className="cart-actions">
            ...
            <Link to="/discover" className="btn btn-ghost">Continue shopping</Link>
          </div>
        </section>

        {/* RIGHT: summary / checkout */}
        <aside className="cart-summary card card--p">
          {/* A11y: maintain heading order under page H1 */}
