...
function AdminOrders({ showMessage: showMessageProp, initialOrders }) {
  ...
  const handleResendInvoice = async (orderId) => {
    try {
      await axios.post(
        `/api/orders/${orderId}/resend-invoice`,
        {},
        { headers },
      );
  ...
  const updateStatus = async (orderId, newStatus) => {
    try {
      ...
      // even if axios baseURL/env differs in some CI runners.
      try {
        ...
        // ignore preflight fallback errors in tests
      }
      await axios.patch(
  ...
  const bulkOrderActions = useBulkOrderActions({
    orders,
    emailTemplate,
    headers,
    BULK_ACTION_LIMIT,
  ...
  return (
    <div style={{ padding: 20 }}>
                ...
                try {
                  const resp = await axios.post(
                    "/api/admin/orders/bulk-status",
                    {
                      ...
                      action: pendingBulkActionType || "Bulk",
                    },
                    { headers },
                  );
                  ...
                  if (success.length === 0 && failed.length >= 0) {
                    ...
                    success = (selectedOrderIds || []).filter(
                      (id) => !failedSet.has(id),
                    );
                  }
                  setLastBulkSummary({
      ...
      <BulkActionPermissionInfo
        show={window?.localStorage?.getItem("adminRole") === "viewer"}
      />
      {/* Bulk export dialog */}
      <BulkExportDialog
        ...
        onConfirm={() => {
          ...
          console.log(
            "[AdminOrders] BulkExportDialog onConfirm called with ids:",
            selectedOrderIds,
          );
        ...
        onPreviewConfirm={() => {
          ...
          // Keep export dialog open after preview confirm; do not show summary yet
          setShowExportDialog(true);
        }}
      ...
      {bulkErrors.length > 0 && (
        <div style={{ marginBottom: 10, color: "red" }}>
          Failed to update: {bulkErrors.join(", ")}
        </div>
      )}
      ...
      <BulkEmailPreviewDialog
        ...
        onConfirm={() => {
          ...
          // eslint-disable-next-line no-console
          console.log(
            "[AdminOrders] BulkEmailPreviewDialog onConfirm called with ids:",
            selectedOrderIds,
          );
        ...
        onPreviewConfirm={() => {
          ...
          // Keep email preview dialog open after preview confirm; do not show summary yet
          setShowEmailPreview(true);
        }}
      ...
      {showBulkSummary && (
        ...
        <>
          {(() => {
            try {
              console.log(
                "[AdminOrders] Rendering BulkSummaryDialog with summary:",
                lastBulkSummary,
              );
            } catch {}
            return null;
