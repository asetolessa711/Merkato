...
function ShopPage() {
  ...
  useEffect(() => {
    // Back-compat redirects to new browse routes
    const sort = params.get('sort');
    const category = (params.get('category') || '').toLowerCase();
    if (sort === 'deals') {
      navigate(LinkBuilder.toDeal('under-25', { category }) || '/discover', { replace: true });
      return;
    }
    // If a discover-like intent, use new category route
    if (category) {
      navigate(LinkBuilder.toCategory(category, { sort: 'best' }) || '/discover', { replace: true });
      return;
    }
