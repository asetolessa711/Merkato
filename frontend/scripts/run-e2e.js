...
async function main() {
  ...
  const backendEnv = { ...process.env, NODE_ENV: 'development', PORT: String(backendPort), npm_config_cache: npmCache, PUPPETEER_CACHE_DIR: puppeteerCache };
  ...
  const cyArgs = ['cypress', 'run', '--config', `baseUrl=http://localhost:${frontendPort}`,'--browser','chrome','--reporter','json','--reporter-options',`output=${reportPath}`,...specArg];
