Scope Lock — TRAN: Customer order-history bounded promotion-window runtime execution and go-or-hold evidence decision

Objective
Execute a real, bounded, reversible customer order-history guarded serving-policy promotion window for covered aliases only, and produce a formal go-or-hold decision artifact from measured runtime evidence.
...
- Bounded-window execution control, evidence capture, and governance decisioning for this slice only.
...
- Any execution-window end state without explicit GO approval reverts to legacy-safe Mongo default posture.
...
- Window timestamps and approved promotion environment identifier are recorded in-slice.
